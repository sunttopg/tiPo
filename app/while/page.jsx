...
const page = () => {
    ...
    const verify = () => {
        if(value === "while(x>0){s=s+x%10; x=x/10}")
    ...
    const rasp = () => {
        alert("Raspunsul corect era while(x>0){s=s+x%10; x=x/10} ");
    }
    ...
    return (
        <main className="flex w-full h-screen justify-center items-center bg-gray-800 p-8">
            <div className='h-[400px] w-[570px] bg-white rounded-lg mt-[7px]'>
                <h1 className='text-5xl font-bold text-center my-[30px] text-primary text-black'>TEST#04</h1>
                <p className="text-xl pt-[10px] pb-[35px] text-black text-center">Calculati suma cifrelor unui numar x, folosind o structura repetitiva cu test initial, stiind ca suma va fi memorata in variabila s *a nu se folosi alte variabile*</p>
